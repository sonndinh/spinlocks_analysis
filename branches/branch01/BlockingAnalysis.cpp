...
void add_prio_constraint_unordered_tiebreak_self(map<ResourceID, vector<IloNumVarArray> > &my_y_vars,
	...
	for (; y_it != my_y_vars.end(); y_it++) {
		...
		int my_req_num = task->myResources[res_id]->requestNum;
...
double max_blocking_fifo(Task *task, map<ResourceID, map<TaskID, CSData> > &x_data) {
	...
	for (; it != x_data.end(); it++) {
		...
		unsigned int my_req_num = task->myResources[res_id]->requestNum;
		double cs_len = task->myResources[res_id]->CSLength;
		...
		for (int i=0; i<=my_req_num; i++) {
			...
			for (map<TaskID, CSData>::iterator inner_it=interfere.begin(); inner_it!=interfere.end(); inner_it++) {
				...
				unsigned int his_proc_num = inner_it->second.procNum;
				unsigned int his_req_num = inner_it->second.requestNum;
				double his_cs_len = inner_it->second.csLen;
...
void sort_by_cslen(Task *task, TaskSet *taskset, map<TaskID, CSData> &interfere, vector<CSData> &results) {
	vector<TaskID> &my_lp_tasks = taskset->lower_prio_tasks[task->taskID];
...
double max_blocking_prio(Task *task, TaskSet *taskset, map<ResourceID, map<TaskID, CSData> > &x_data,
	...
	vector<TaskID> &my_lp_tasks = taskset->lower_prio_tasks[task->taskID];
	vector<TaskID> &my_ep_tasks = taskset->equal_prio_tasks[task->taskID];
	vector<TaskID> &my_hp_tasks = taskset->higher_prio_tasks[task->taskID];
	unsigned int my_proc_num = task->procNum;
	double my_blocking = 0;
	...
	for (; it != x_data.end(); it++) {
		...
		for (int i=0; i<=my_req_num; i++) {
			...
			for (int i=0; i<sorted_by_cslen.size(); i++) {
				if (sorted_by_cslen[i].requestNum >= remain_req) {
					tmp_blocking += remain_req * sorted_by_cslen[i].csLen;
					break;
				} else {
					tmp_blocking += sorted_by_cslen[i].requestNum * sorted_by_cslen[i].csLen;
					remain_req -= sorted_by_cslen[i].requestNum;
				}
			}
			...
			if (lock_type == PRIO_UNORDERED) {
				...
				for (; inter_it != interfere.end(); inter_it++) {
					...
					if (find(my_ep_tasks.begin(), my_ep_tasks.end(), his_task_id) != my_ep_tasks.end() ||
						...
						unsigned int req_inside_dpr = njobs(tset[his_task_id], dpr_for_curr_res)*tset[his_task_id]->myResources[res_id]->requestNum;
